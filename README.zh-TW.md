# ezshot

**語言：** 繁體中文 | [English](README.md)

Windows 螢幕截圖工具，常駐系統匣，支援全域快捷鍵、截圖後即時標註與裁切。

## 功能

### 截圖模式

| 快捷鍵 | 模式 |
|--------|------|
| `Alt+Shift+R` | 拖曳框選區域 |
| `Alt+Shift+F` | 擷取全螢幕 |
| `Alt+Shift+A` | 擷取目前作用中視窗 |
| `Alt+Shift+W` | 點選任意視窗 / 控制項 |

視窗擷取使用 DWM 可見視窗邊界，避免把不可見的 resize 外框一起截入。

### 延遲擷取

系統匣右鍵選單可設定延遲（0 / 1 / 2 / 3 / 5 秒或自訂），方便擷取懸停選單、tooltip 等瞬間狀態。  
倒數期間螢幕中央顯示計時數字，並以橘色框標示即將擷取的區域。

### 編輯器

截圖後開啟多分頁編輯器：

| 工具 | 說明 |
|------|------|
| 筆 | 自由繪製 |
| 箭頭 | 繪製箭頭 |
| 矩形 | 繪製矩形框 |
| 文字 | 點擊後輸入文字 |
| 裁切 | 拖曳選取區域後立即裁切（可復原） |
| 馬賽克 | 拖曳選取區域套用馬賽克遮蓋（可復原） |
| 顏色 / 粗細 | 下拉面板選色（12 預設色 + 系統調色盤）；內含粗細輸入框與線條預覽 |
| 複製 | 複製到剪貼簿 |
| 儲存 | 首次開對話框（預設檔名為時間戳記）；之後直接覆蓋 |
| 另存 | 另存至新位置（預設帶入現有檔名；需先存過一次） |
| 復原 | 復原最後一筆畫或裁切 |
| ≡ 設定 | 開啟設定選單（與系統匣設定同步） |

#### 鍵盤快捷鍵

| 快捷鍵 | 動作 |
|--------|------|
| `Alt+P` | 筆 |
| `Alt+A` | 箭頭 |
| `Alt+R` | 矩形 |
| `Alt+T` | 文字 |
| `Alt+C` | 裁切 |
| `Alt+M` | 馬賽克 |
| `Ctrl+Z` | 復原 |
| `Ctrl+C` | 複製到剪貼簿 |
| `Ctrl+S` | 儲存 |
| `Ctrl+Shift+S` | 另存新檔 |

- 每次截圖開一個新**分頁**，分頁名稱為截圖時間戳記（`YYYYMMDDhhmmss`）
- 視窗標題顯示 `ezshot-<分頁名稱>`
- 有未存修改的分頁顯示紅點警示；存檔後消失
- 點分頁右側 × 關閉單一分頁；若未存檔會跳出扁平確認對話框（儲存 / 不儲存 / 取消）
- 點視窗 × 關閉按鈕：依序詢問所有未存分頁，清除所有分頁後隱藏到系統匣
- 點最小化按鈕：縮小到工作列，保留所有分頁
- 只有從系統匣右鍵功能表選擇結束才會真正關閉程式
- 分頁過多時自動捲動，新截圖永遠可見

### 設定（系統匣右鍵 / 工具列 ≡ 按鈕）

- **擷取滑鼠游標**：截圖是否包含游標
- **直接複製到剪貼簿**：擷取後自動複製至剪貼簿，再開啟編輯器
- **擷取前隱藏編輯視窗**：按快捷鍵時先隱藏編輯視窗再截圖
- **延遲擷取**：倒數秒數（支援自訂）
- **關閉所有頁籤**：依序詢問未存分頁（儲存 / 不儲存 / 通通不存檔 / 取消），確認完畢後隱藏視窗
- 設定自動儲存至 `%APPDATA%\ezshot\settings.ini`
- 儲存路徑記憶至 `%APPDATA%\ezshot\last_dir.txt`

## 系統需求

- Windows 10 / 11（x64）
- Rust toolchain（用於從原始碼建置）

## 建置

```powershell
cargo build --release
```

產生的執行檔位於 `target\release\ezshot.exe`，無需安裝，直接執行即常駐系統匣。

## 技術

純 Rust + Win32 API（`windows` crate 0.58），不依賴 .NET 或第三方 UI 框架。
